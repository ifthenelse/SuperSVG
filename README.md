...
Use the linear RunPod flow in section 8.1.

Milestone 1 is reached when all are true:

- Training run completes without crashing
- Checkpoint exists in `/workspace/supersvg/output_coarse/checkpoints.pt` (or your overridden persistence path)
- Sample output images exist in `/workspace/supersvg/output_coarse/imgs/`
- Training logs exist in `/workspace/supersvg/output_coarse/log.txt`
...
## 8.1 RunPod linear setup + run flow (recommended)

Follow these steps in order, without branching.

### Step 1 — Connect to pod (from local machine)

```bash
ssh root@<POD_PUBLIC_IP> -p <POD_PORT> -i ~/.ssh/id_runpod_ed25519
```

Use values from the RunPod pod console.

If SSH fails:

- Disable VPN and retry
- Regenerate keypair in RunPod console and retry
- As fallback, use password auth once: `ssh root@<POD_PUBLIC_IP> -p <POD_PORT>`

### Step 2 — Configure GitHub access on pod (private repo only)
...
### Step 4 — Run one-time machine setup

```bash
./setup-lambda-labs.sh
```

This creates launchers `~/train_supersvg.sh` and `~/monitor_training.sh`, and supports both Docker-enabled and container-mode RunPod templates automatically.
...
```bash
/root/train_supersvg.sh
```

Defaults used by launcher:

- `DATA_PATH=~/SuperSVG/input/test`
- `BATCH_SIZE=4` (safe for 5-image test dataset)
- Persistent root: `/workspace/supersvg` when `/workspace` is mounted; otherwise `$HOME/supersvg_persistent`

For larger datasets:

```bash
DATA_PATH=/path/to/dataset BATCH_SIZE=32 /root/train_supersvg.sh
```

### Step 7 — Verify artifacts were persisted

```bash
ls -lh /workspace/supersvg/output_coarse/checkpoints.pt
ls -lh /workspace/supersvg/output_coarse/log.txt
ls /workspace/supersvg/output_coarse/imgs | tail -n 5
```

If you override `PERSIST_ROOT`, check that location instead.

### Step 8 — Stop pod when done

Terminate or stop the pod in RunPod UI to avoid idle charges.

### RunPod container-mode note

If shell prompt looks like `root@<container-id>`, you are in container mode. `systemd` and `docker restart` commands may fail; this is expected. The setup script already handles this mode and falls back to direct `micromamba` execution when needed.
